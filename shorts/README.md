...
- Window entrances never repeat within a video (the `entrance()` map in Short.tsx): hook = CRT power-on, found = slide from the right, chart = maximise from the bottom-left, props = drop from the top, timeline = wipe left to right, outro = zoom with a twist. Add a new entrance when adding a new scene kind.
...
Word timings are estimated with the Mac voice (exact with ElevenLabs). Wanted next: a receipt scene for price-then-vs-now posts, a "loading" scene for websites, background music bed at low volume, automatic upload (YouTube Data API once Pedro creates the channel and pastes credentials into GitHub secrets).
